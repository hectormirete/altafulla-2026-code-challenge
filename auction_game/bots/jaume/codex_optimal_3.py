...
class CodexOptimal3Bot(AuctionBot):
    """Conservative score-differential bidder for the auction challenge."""
    ...
    def _raise_if_worthwhile(
        ...
        *,
        ...
        if required <= self._ceiling(state, opponent_bid=opponent_bid, final_round=final_round):
            return required
        return min(my_bid, state.my_budget)
